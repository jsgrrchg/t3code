...
export type RunThreadProviderActionInput = CommandInput<"thread.provider-action.run">;
...
export const runThreadProviderAction: (input: RunThreadProviderActionInput) => CommandEffect =
  Effect.fn("EnvironmentCommands.runThreadProviderAction")(function* (input) {
    const metadata = yield* timestampedCommandMetadata(input);
    return yield* dispatch({
      ...input,
