...
function PullRequestsRouteView() {
  ...
  const reorderSurface = (surfaceId: string, overSurfaceId: string) => {
    if (rightPanelRef === null) return;
    useRightPanelStore.getState().moveSurface(rightPanelRef, surfaceId, overSurfaceId);
  };
  const closeSurface = (surface: PullRequestSurface): Promise<boolean> => {
    if (rightPanelRef === null) return Promise.resolve(false);
  ...
  const closeOtherSurfaces = (surface: PullRequestSurface): Promise<boolean> => {
    if (rightPanelRef === null) return Promise.resolve(false);
    useRightPanelStore.getState().closeOtherSurfaces(rightPanelRef, surface.id);
    selectSurfaceInUrl(surface);
    return Promise.resolve(true);
  };
  const closeSurfacesToRight = (surface: PullRequestSurface): Promise<boolean> => {
    if (rightPanelRef === null) return Promise.resolve(false);
  ...
  const closeAllSurfaces = (): Promise<boolean> => {
    if (rightPanelRef === null) return Promise.resolve(false);
      ...
      <div className="relative flex min-h-0 flex-1">
        ...
        {rightPanelState.isOpen && activePullRequestSurface && panelEnvironmentId !== null ? (
          <RightPanelTabs
            ...
            onRegenerateChatTitle={() => undefined}
            onDeleteChat={() => Promise.resolve(false)}
            browserAvailable={false}
            terminalAvailable={false}
            diffAvailable={false}
            historyAvailable={false}
            historyDisabledReason="History is unavailable in the pull-request workspace."
            filesAvailable={false}
            chatAvailable={false}
            chatTitlesById={EMPTY_TERMINAL_LABELS}
            panelChats={[]}
            chatOpenAnnouncement={null}
            onChatOpenAnnouncementHandled={() => undefined}
            chatTitleRegenerationAvailable={false}
