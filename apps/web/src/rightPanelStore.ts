...
// v13 combines pull-request surfaces with the remix's durable surface model.
...
interface RightPanelStoreState {
  ...
  open: (
    ref: ScopedThreadRef,
    kind: Exclude<RightPanelKind, "file" | "terminal" | "chat" | "git-commit" | "pull-request">,
  ) => void;
  ...
  moveSurface: (ref: ScopedThreadRef, surfaceId: string, overSurfaceId: string) => void;
...
const singletonSurface = (
  kind: Exclude<
    RightPanelKind,
    "file" | "preview" | "terminal" | "chat" | "git-commit" | "pull-request"
  >,
): RightPanelSurface => {
  switch (kind) {
    case "diff":
...
export const useRightPanelStore = create<RightPanelStoreState>()(
  persist(
    (set) => ({
      ...
      moveSurface: (ref, surfaceId, overSurfaceId) =>
        set((state) => ({
          byThreadKey: updateThread(state.byThreadKey, scopedThreadKey(ref), (current) => {
            const fromIndex = current.surfaces.findIndex((surface) => surface.id === surfaceId);
            const toIndex = current.surfaces.findIndex((surface) => surface.id === overSurfaceId);
