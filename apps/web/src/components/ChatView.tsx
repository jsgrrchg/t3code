...
function ChatViewContent(props: ChatViewProps) {
  ...
  const isWorkspacePresentation = chatPresentationOwnsWorkspaceChrome(presentation);
  ...
  const reorderRightPanelSurface = useCallback(
    (surfaceId: string, overSurfaceId: string) => {
      if (!activeThreadRef) return;
      useRightPanelStore.getState().moveSurface(activeThreadRef, surfaceId, overSurfaceId);
    },
    [activeThreadRef],
  );
  ...
  }, [isWorkspacePresentation, togglePreviewPanel]);
  ...
  return (
    ...
    >
        ...
        {isWorkspacePresentation ? (
          <header
            ...
            className={cn(
              ...
              isElectron
                ? cn(
                    "workspace-topbar drag-region relative flex h-[var(--workspace-topbar-height)] min-h-[var(--workspace-topbar-height)] shrink-0 items-center px-3 sm:px-5",
                    reserveTitleBarControlInset &&
                      !inlineRightPanelOwnsTitleBar &&
                      "wco:pr-[var(--workspace-native-controls-inset)]",
                  )
                : "workspace-topbar flex h-[var(--workspace-topbar-height)] min-h-[var(--workspace-topbar-height)] shrink-0 items-center pl-[calc(env(safe-area-inset-left)+0.75rem)] pr-[calc(env(safe-area-inset-right)+0.75rem)] sm:pl-[calc(env(safe-area-inset-left)+1.25rem)] sm:pr-[calc(env(safe-area-inset-right)+1.25rem)]",
      ...
      {isWorkspacePresentation && !shouldUseRightPanelSheet && rightPanelOpen && activeThreadRef ? (
        <RightPanelTabs
          mode="inline"
          maximized={rightPanelMaximized}
