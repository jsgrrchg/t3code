...
Queued work appears above the composer. Drag an entry by its handle to change when it runs; the handle also supports keyboard reordering. Edit a queued message directly in its row, remove it if it is no longer needed, or select **Steer** to send it to the active turn immediately. While a turn is running, hold Command on macOS or Control on Windows and Linux to reveal the Steer shortcut on the last queued message. Press Enter while continuing to hold the modifier to steer that message when the composer is empty; composer content always keeps its normal send or queue behavior. While a message is being edited, automatic queue dispatch pauses until you save or cancel. The queue is stored locally and survives an app restart.
